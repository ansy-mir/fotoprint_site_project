...
var drop = function drop() {
  // drag *
  // dragend *
  // dragenter - объект перетаскивается НАД dropArea
  // dragexit *
  // dragleave - объект перетаскиваеют ЗА пределами dropArea
  // dragover - объект зависает над dropArea и двигается в его пределах
  // dragstart *
  // drop - пользователь отпускает мышь и перетаскиваемый объект "падает" в dropArea
  // * - события срабатывают только с перетаскиваемыми элементами НА странице. А не с подгружаемыми извне.
...
var filter = function filter() {
  ...
  var addEventLisToMark = function addEventLisToMark(argument) {
    var argBtn = menu.querySelector(argument);

    if (argument === '.grandmother' || argument === '.granddad') {
      argBtn.addEventListener('click', function () {
        typeFilter();
      });
    } else {
      var mark = wrapper.querySelectorAll(argument);
      argBtn.addEventListener('click', function () {
        typeFilter(mark);
      });
    }
  ...
  addEventLisToMark('.all');
  addEventLisToMark('.lovers');
  addEventLisToMark('.chef');
  addEventLisToMark('.girl');
  addEventLisToMark('.guy');
  addEventLisToMark('.grandmother');
  addEventLisToMark('.granddad');
...
/* harmony import */ var _servises_requests__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! ../servises/requests */ "./src/js/servises/requests.js");
...
var forms = function forms(state) {
  ...
  allForms.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.parentNode.appendChild(statusMessage); //Красиво скрыть форму, чтобы на её месте появилось окно оповещения. 
      ...
      console.log(JSON.stringify(state)); //Переменная для формирования динамического пути для отправки данных

      var api; //closest возвращает ближайший родительский элемент (или сам элемент), который соответствует заданному CSS-селектору 
      ...
      console.log(api);
      Object(_servises_requests__WEBPACK_IMPORTED_MODULE_7__["postData"])(api, formData).then(function (result) {
        console.log(result);
        ...
        }, 4000); //Закрываем окно после ввода всех данных
        // if (item.getAttribute('data-calc') === 'end') {
        //   setTimeout(() => {
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    ...
    console.log(val); //this.value - то, что ввёл пользователь. /./g - проходим по каждому символу из матрицы (a) и применяем к нему функцию
...
var modals = function modals() {
  ...
  function bindModals(triggerSelector, modalSelector, closeSelector) {
    ...
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        ...
        openModal.style.display = 'block'; //Для блокировки скролла страницы при открытом модальном окне
    ...
    openModal.addEventListener('click', function (e) {
      if (e.target === openModal) {
        ...
        openModal.style.display = 'none';
        document.body.style.overflow = '';
        document.body.style.marginRight = '0px'; // clearInterval(showModalByTime);
      }
    });
  }
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var isAnyModalShown = false; //Проверяем, показывается ли пользователю какое-то из модальных окон
  ...
  openByScroll('.fixed-gift'); // showModalByTime('.popup-consultation', 70000);
};
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(blocks) {
    var img = blocks.querySelector('img');
...
var scrolling = function scrolling(upSelector) {
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (event) {
      ...
      var heigthTop = document.documentElement.scrollTop,
          hash = this.hash,
      ...
      function step(time) {
        ...
        var progress = time - start,
            r = toBlock < 0 ? Math.max(heigthTop - progress / speed, heigthTop + toBlock) : Math.min(heigthTop + progress / speed, heigthTop + toBlock);
        document.documentElement.scrollTo(0, r);

        if (r != heigthTop + toBlock) {
          requestAnimationFrame(step);
        } else {
          location.hash = hash;
        }
  ...
  //       clearInterval(move);
  //       history.replaceState(history, document.title, location.href.replace(/#.*$/g, '') + hash);
  //     } else {
...
/* harmony import */ var _servises_requests__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../servises/requests */ "./src/js/servises/requests.js");
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_servises_requests__WEBPACK_IMPORTED_MODULE_3__["getResource"])('http://localhost:3000/styles').then(function (result) {
